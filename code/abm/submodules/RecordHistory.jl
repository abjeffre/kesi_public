function RecordHistory(; history = history, K = K, kmax = kmax, ngroups = ngroups, effort = effort,
     ...
     rec_history = rec_history, full_save = full_save, HG = HG, loc = loc, caught2 = caught2,
      n = n, sim=sim, year = year, WL)
    history[:stock][year,:,sim] .=round.(K./kmax, digits=3)
    for col in 1:size(effort, 2)
        history[:effort][year, col, :, sim] .= round.(report(effort[:, col], agents.gid, ngroups), digits=3)
    end
    for col in 1:size(WL, 2)
        history[:wages][year, col, :, sim] .= round.(report(WL[:, col], agents.gid, ngroups), digits=3)
    end
    ...
    history[:effortfull][year, :,:, sim] .=  effort
    ...
    # history[:cel][year,:,sim] .=  round.(reportCor(effort[:,1], traits.harv_limit,agents.gid, ngroups), digits=3)
    # history[:clp2][year,:,sim] .= round.(reportCor(effort[:,1], traits.punish_type2,agents.gid, ngroups), digits=3)
    # history[:cep2][year,:,sim] .= round.(reportCor(traits.harv_limit, traits.punish_type2,agents.gid, ngroups), digits=3)
    # history[:ve][year,:,sim] .= round.(reportVar(effort[:,1],agents.gid, ngroups), digits=3)
    ...
    #  history[:fstEffort][year,sim] = GetFST(effort[:,1], agents.gid, ngroups, experiment_group, experiment)
