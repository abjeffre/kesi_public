function getHistoryBook(;n = n, nrounds = nrounds, ngroups = ngroups,
     ...
     rec_history = rec_history, population_growth = population_growth, ngoods = ngoods)
    history=Dict{Symbol, Any}(
    :stock => zeros(nrounds, ngroups, nsim),
    :effort => zeros(nrounds, ngoods, ngroups, nsim),
    ...
    :wages => zeros(nrounds, ngoods, ngroups, nsim))
    

  if full_save == true 
    history[:limitfull] = zeros(nrounds, n, nsim)
    history[:effortfull] = zeros(nrounds, n,ngoods, nsim) 
