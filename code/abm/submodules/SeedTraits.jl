...
######################################################
############ IF we have multipe seasons ##############

function SeedTraits(; n=n, ngroups=ngroups, agents = agents,  ngoods=ngoods,
  gs_init = gs_init, limit_seed = limit_seed, harvest_var = harvest_var,
  harvest_var_ind = harvest_var_ind, seed = seed, fine_start = fine_start, 
  learn_group_policy =learn_group_policy, invasion = invasion, fine_var = fine_var, 
  effort_seed = effort_seed, limit_seed_override=limit_seed_override, nperiods = nperoids)
...
temp = ones(ngoods)
 ...
 # here we will introduce seasons by having a vector that carries diffrent effort values
 # at each time step we will check the season and deploy the relevent effort

 period_efforts = fill(effort, nperiods)
 ...
 return effort, traits, traitTypes, traitTypesGroup, period_efforts
end
