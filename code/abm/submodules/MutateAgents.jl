...
function MutateAgents(trait, mutation, types)
    ...
    if types == "Dirichlet"
        temp = Matrix(x) .* 5 .+ 0.01
        for i in 1:n
            if trans_error[i, 1]
                x[i, :] .= rand(Dirichlet(temp[i, :]))
            end
        end
    else
        for i in 1:k
            if types[i] == "binary"
                x[:, i] .= ifelse.(trans_error[:, i], 1 .- x[:, i], x[:, i])
            elseif types[i] == "prob"
                n_error = rand(Normal(), n)
                proposal = clamp.(inv_logit.(logit.(x[:, i]) .+ n_error), 0.001, 0.999)
                x[:, i] .= ifelse.(trans_error[:, i], proposal, x[:, i])
            elseif types[i] == "positivecont"
                n_error = rand(Normal(0, 0.2), n)
                proposed = x[:, i] .+ n_error
                x[:, i] .= ifelse.(trans_error[:, i], ifelse.(proposed .<= 0, 0.1, proposed), x[:, i])
            end
        end
    end
