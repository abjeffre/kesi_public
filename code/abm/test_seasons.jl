...
function cpr_abm(
  ...
  congestion_alpha = 0,                # Is used to scale congestion effects
  ...
  length(wages) !=ngoods-1 ? println("Check to make sure you are defining wages correctly") : nothing
  ...
  rec_history = rec_history, population_growth = population_growth, ngoods = ngoods) # Make history books
  ...
  for sim = 1:nsim
    ...
    # Seed all traits
    effort, traits, traitTypes, traitTypesGroup, period_efforts = SeedTraits(n=n, ngroups=ngroups, agents = agents,  ngoods=ngoods,
                                                            ...
                                                            limit_seed_override = limit_seed_override, effort_seed = effort_seed, nperiods = nperiods)
    ...
    # Construct Periods 
    periods = reduce(vcat,fill(collect(1:1:nperiods), Int(nrounds/nperiods )))
    ...
    for t ∈ 1:nrounds
      ...
      effort, traits = RunExperiment(;experiment = experiment,
       ...
       if experiment_price != false
          price=Float64.(fill(price_copy, n))
          price[agents.gid .∈ experiment_group] .= experiment_price
      end
      if verbose== true print(string("Experiment: COMPLETED"))end      
      ####################
      #### Politics #######
      # NEEDS SPEED INCREASE!
      ...
      if catch_before == true
        ...
        temp_effort = effort[:,1] .* (1 .-caught1)
        ...
        if harvest_type == "collective"
          GH=GetGroupHarvest(effort[:,1], loc, K, kmax, tech, labor, degrade, ngroups)
          HG=GetIndvHarvest(GH, effort[:,1], loc, necessity, ngroups)
        else
          HG=GetHarvest(effort[:,1], loc, K, kmax, tech, labor, degrade, necessity, ngroups, agents)
      ...
      pollution ? POL =  GetPollution(effort[:,1], loc, ngroups, pol_slope, pol_C, K, kmax) : POL = zeros(n)  # ALLOWS FOR ADDITIONAL NEGATIVE EXTERNALITIES FROM LOW QUALITY RESOURCE STOCK    
      
  
      ##################################
      ###### PAYOFFS ###################
      #Wage Labor Market
      if wage_data !== nothing wages = Vector(wage_data[t,:]) end
      if labor_market == false 
        WL = GetWages(effort[:,2:ngoods], wages, wage_elasticities) 
      else
        WL = GetWages(effort[:,2:ngoods], wages, agents.gid, labor_market, ngroups, wage_elasticities) 
      end
      if eco_sys_dam !== nothing
        WL = GetWages(effort[:,2:ngoods], wages, agents.gid, ngroups, K, kmax, eco_sys_dam, wage_elasticities)
      end
      # WL[WL .< 0] .=0
      agents.payoff_round = 
      ...
      if verbose == true
        ...
        println("TC,", sum(isnan.(TC)))
        println("Congestion", sum(isnan.(getCongestion(loc, effort[:,1], agents.gid, ngroups, congestion_alpha))))
      end
      ...
      if sly == true # If social learning year (SLY) is true then do the social learning dangit!
        if n == ngroups # This for when the number of individuals equals the number of groups - niche conditions where there is one person per group.  
          ...
          if year > 1 # No learning on Year 1. 
            ...
            #println("Before: ", sum(effort[:,1][models] .>= effort[:,1][agents.id]))
            ...
            # RIGHT NOW I AM LITTLE CONFUSED AS TO WHY THIS ISN'T SIMPLY SOCAIL TRANSMISSION 1
            # THIS IS A VERY SLOW FUNCTION!
            effort=SocialTransmission2(effort, models, fidelity, "Dirichlet")
