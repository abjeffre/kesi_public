...
data {
  ...
  real softplus_alpha;
  int<lower=1, upper=K> clove_sector; // column of y holding clove earnings
  vector[N_est] clove_hist; // official clove earnings for the imputed periods
}
parameters {
    ...
    for (k in 1 : K) {
      ...
      for(m in 1:M){
        ...
        gdp_impute_mu[i, k] = softplus( 
                                  ...
                                  *scale_cpi[est_ind[i]], softplus_alpha);
    }
  }
...
model {
  ...
  // Income effect
  for(i in 1:K) bgdp[i] ~ normal(0, 1);
  // Gamma Scale
  ...
  to_vector(z_ver) ~ normal(0, 2);
  b_sver_kesi ~ normal(0, 1);
  sigma_sver_kesi ~ exponential(1);
  a ~ normal(22, 2); // Note this prior is chosen given testing using frequentist models to determine likely range. 
  omega ~ exponential(1);
  ...
  // Robustness check 2: official clove statistics replace the imputed clove column
  for(i in 1:N_est){
    gdp_merged[i, clove_sector] = clove_hist[i];
  }
