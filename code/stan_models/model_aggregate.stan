...
model {
  ...
  a ~ normal(20, 2); // Note this prior is chosen given testing using frequentist models to determine likely range. 
  omega ~ exponential(1);
  mu_gdp ~ normal(0, 3);
  ...
  for(i in 1:N_est){
          gdp_merged[i] = sum(gdp_impute[i,]);
  }

  
  // Assign Real Values to Merged
  for(i in 1:N_eco){
      gdp_merged[eco_ind[i]]=sum(gdp_true[ i , ]);
  }
  for (i in 1 : N_cases) {
